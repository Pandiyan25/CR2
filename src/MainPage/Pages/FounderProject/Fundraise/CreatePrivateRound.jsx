...
const CreatePrivateRound = ({ tokenStd1,getFundingRound, handleClose, founderDashBoard, TokenomicsPage }) => {
    ...
    const setChooseMileStonedataFunc = (index1, evnt) => {
        ...
        // console.log("1234 Data",chooseMileStonedata)
        var updatedData = chooseMileStonedata.map((x, index) => (index === index1 ? 
            {
                ...x,
            choose: value,
        ...
        // console.log("1234 Data Updated",updatedData);
          ...
          'x-range-name': process.env.RANGE_NAME
                ...
                .then(data => {
                    ...
                    if (data?.data?.allTokenomics != null && data?.data?.allTokenomics != undefined && data?.data?.allTokenomics.length > 0) {
                        ...
                        alert("Please fill out the Tokenomics section before continuing with fundraising.")
                        TokenomicsPage()
                        setTokenomicsData([])
                    ...
                    if (data?.data?.allProjectFundingData != null && data?.data?.allProjectFundingData != undefined) {
                        setProjNameValue(data?.data?.allProjectFundingData[0]?.project?.project_name)
                    } else {
                        setProjNameValue('')
                    }
                })
    ...
    const changeValueForChoose = (index1, evnt) => {
        const { name, value } = evnt.target;
        // console.log("1234 Data",chooseMileStonedata)
        const updatedData = chooseMileStonedata.map((x, index) => 
        (index === index1 ?
        ...
        try {
            ...
            if (chooseMileStonedata.length > 0) {
                ...
                if (choosefiliter < 0) {
                    ...
                    console.log(mainSum,sum,parseFloat(initialRelease),"mileStoneArray");
    ...
    const milestonecreateFunc = (fundRaiseMainid, mailstoneData) => {
        try {
            var query = `
          ...
          'x-range-name': process.env.RANGE_NAME
                ...
                body: JSON.stringify({
                    ...
                    variables: {
                        "input": {
                            "fundraise": fundRaiseMainid,
                            "milestone": mailstoneData.milestone,
                            "percentage": parseFloat(mailstoneData.percentage),
                            "milestone_status": 'Yet_to_start',
                            "target_date": mailstoneData.target_date,
                            "validation_status": 'Unrequested',
                            "funds": 0,
                            "remarks": '',
                            "estimated_target_date": '',
                ...
                .then(data => {
                    // console.log('getFounderUserDetails', data?.data?.allProjects);
                    if (data?.data?.createMilestone != null && data?.data?.createMilestone != undefined) {
    ...
    return (
        <div className="row">
                ...
                <div className="row" style={{ padding: '10px' }}>
                    <div className="col-md-12" style={{ marginBottom: '1rem', display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                        ...
                        <div style={{ width: '300px' }}>
                            <select className="form-control btn-block-height square-edges" onChange={(e) => setIvestor(e.target.value)} >
                                <option style={{ fontSize: '13px' }} value="" >Select Investor</option>
                                {   
                                    (tokenStd1.map((i,index) => (

                                        <option style={{ fontSize: '13px' }} value={i?._id} key={index}  >{`${i?.fund_name} `} </option>
                                    )))
                                }
                            </select>
                        </div>
                         : <div>
                         <button className="btn btn-primary " onClick={founderDashBoard}>Connect with Investors</button>   
                        </div>}
                    </div>
                    <div className="col-md-12" style={{ marginBottom: '1rem', display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <div className="col-md-12" style={{ marginBottom: '1rem', display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                        ...
                        <div style={{ width: '300px' }}>
                            ...
                            {/* type="number" className="form-control" style={{ width: '300px' }} value={FundRequested} onChange={(e) => fundReqFunc(e.target.value)} /> */}
                    ...
                    <div className="col-md-12" style={{ marginBottom: '1rem', display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                        ...
                        <div style={{ width: '300px' }}>
                            {/* onChange={(e) => setExpFreq(e.target.value)} onWheel={(e) => e.target.blur()} */}
                            <input type="text" disabled="true" className="form-control" style={{ width: '300px' }} readOnly='true' value={TokenTicker} onChange={(e) => setTokenTicker(e.target.value)}
                    ...
                    <div className="col-md-12" style={{ marginBottom: '1rem', display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                        ...
                        <div style={{ width: '300px' }}>
                            ...
                            {/* <Input onKey /> */}
                            {/* <input onKey="return isNumberKey(event)" type="text" /> */}
                            <input 
                    ...
                    <div className="col-md-12" style={{ marginBottom: '1rem', display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                        ...
                        <div style={{ width: '300px' }}>
                            {/* onChange={(e) => setExpFreq(e.target.value)} onWheel={(e) => e.target.blur()} */}
                            <input disabled="true" type="text" className="form-control" style={{ width: '300px' }} value={TokenomicsData.length > 0 && TokenomicsData[0].total_token_supply} readOnly='true' />
                    ...
                    <div className="col-md-12" style={{ marginBottom: '1rem', display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                        ...
                        <div style={{ width: '300px' }}>
                            {/* onChange={(e) => setExpFreq(e.target.value)} onWheel={(e) => e.target.blur()} */}
                            <input disabled="true" type="text" className="form-control" style={{ width: '300px' }} value={Valuation} readOnly='true' />
                    ...
                    <div className="col-md-12" style={{ marginBottom: '1rem', display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                        ...
                        <div style={{ width: '300px' }}>
                            ...
                            <select className="form-control btn-block-height square-edges" value={initialRelease} onChange={(e) => setInitialReleasefunc(e.target.value)} >
                                {Count.map((_, i) => (
                                    <option style={{ fontSize: '13px' }} value={i + 1} key={i} >{`${i + 1}%`}</option>
                                            ...
                                            <td style={{ width: '25%', padding: '10px' }}>
                                                {
                                                    i.choose == 'Choose_MileStone' ?
                                                        ...
                                                        <div>
                                                            <select className="form-control btn-block-height square-edges" value={i?.milestone} onChange={(evnt) => changeValueForChoose(index, evnt)}>
            ...
            <ToastContainer
                ...
                pauseOnHover
            />
            <CongPopupLaunch PublicRound='You have Successfully Created a Private Round' show={showPopupLaunch} handleClose={handleCloseShowPopup} />
        </div>

    )
}
export default CreatePrivateRound;
