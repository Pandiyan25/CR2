...
const [ToastPart,setToastPart] = useState(true)
    ...
    const getInvestorDetails = () => {
        try {
            var query = `
            query Query($role: String, $user: ID, $connected: Boolean) {
    ...
    //         let [month, day, year] = maindate.split("-");
       
    //         let newdate = `${day}-${month}-${year}`;
       
    //         setVestingLinearStartDate(newdate)
    //         setVestingLinearStartDate1(maindate1)
    //         setVestingLinearEndDate(newdate)
    //         setVestingMonths(0)
    //     } else {
  ...
  date.setMonth(date.getMonth() + parseInt(e));
  setVestingLinearStartDate(date.toISOString().substring(0, 10));
  setVestingLinearEndDate(date.toISOString().substring(0, 10));
    }
    ...
    const setTgefunc = (i) => {
        ...
        var maindate = `${date.getFullYear()}-${date.getMonth()}-${date.getDate()}`;

        var maindate1 = `${date.getFullYear()}-${date.getMonth()}-${date.getDate()}`;
        console.log(maindate, "maindate setTge");

        console.log(maindate, "maindate setTge");
        let [year, month, day, ] = maindate.split("-");
    ...
    const setEndVestingDateFunc = (i) => {
            ...
            if (vestingMode === "Daily") {
              endDate.setDate(startDate.getDate() + parseInt(i));
            } else if (vestingMode === "Monthly") {
              endDate.setMonth(startDate.getMonth() + parseInt(i));
            }
            setVestingLinearEndDate(endDate.toISOString().substring(0, 10));
            setVestingMonths(i)
...
//         }
//         console.log(maindate, "maindate vestingperiod");
                                ...
                                <div className="row" >
                                    ...
                                    <div className="col-md-6 col-sm-6 col-lg-6 paddingDashboard" style={{ padding: '0px' }} >
                                        <div className="col-sm-12">
                                            ...
                                            <div className="row" style={{ margin: '10px 0px 10px 0px' }}>
                                                {releaseMode == 'Linear' ?
                                                    <input type="number" placeholder='Enter Cliff Months' className="form-control" value={cliffMonths} onChange={(e) => setCliffMonthsFunc(e.target.value)} min={0}/>
                                            ...
                                            <div className="row" style={{ margin: '10px 0px 10px 0px' }}>
                                                {releaseMode == 'Linear' ?
                                                  <input type="number" placeholder='Enter Vesting Period' className="form-control" value={vestingMonths} onChange={(e) => setEndVestingDateFunc(e.target.value)} min={0} />
