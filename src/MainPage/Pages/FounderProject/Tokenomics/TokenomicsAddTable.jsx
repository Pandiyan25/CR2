...
function TokenomicsAddTable({ show, DataforPopup, getTokenDetailFunc, handleClose }) {

    const [percentage, setPercentage] = useState(0)
    const [noOfTokens, setnoOfTokens] = useState(0)
    const [TokenPrice, setTokenPrice] = useState(0)
    const [Valuation, setValuation] = useState(0)
    const [TgeRel, setTgeRel] = useState(0)
    ...
    const createTokenomics = () => {
        ...
        try {
          ...
          'x-range-name': process.env.RANGE_NAME
                ...
                .then(data => {
                    if (data?.data?.createTokenomicsData != null && data?.data?.createTokenomicsData != undefined) {
                        getTokenDetailFunc()
...
export default TokenomicsAddTable;